...
#     else
#       { error: "Error: Failed to get coordinates for #{address} - #{data['status']}" }
#     end
#   end

#   # Function to get travel time using Routes API
...
#     return origin_coords if origin_coords[:error]  # Return error if invalid origin
#     return destination_coords if destination_coords[:error]  # Return error if invalid destination
...
#       duration_minutes = (duration_seconds.to_f / 60.0).round
#       distance_meters = data["routes"][0]["distanceMeters"]
#       distance_km = (distance_meters.to_f / 1000).round(2)
